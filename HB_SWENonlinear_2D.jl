...
# pdes = [Dt(eta) + height * Dx(u) * (height + eta) + height * Dy(v) * (height + eta) - 0.5*(Dx(Dx(eta)) + Dy(Dy(eta))),
#         Dt(u) + g * Dx(eta) + u * Dx(u) + v * Dy(u),
#         Dt(v) + g * Dy(eta) + u * Dx(v) + v * Dy(v)];
...
# DiffMat, LaplCoeff = create_jac_blocks(equations, var_exprs, harmonics, Nvar)
...
gif(anim, "SWE_noJac_nonlinear.gif", fps=25)
